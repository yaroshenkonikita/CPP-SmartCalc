...
void CreditCalc::GetValuesFromData() {
  if (data.month_pay_first == data.month_pay_last) {
    ui->monthPaymentLabelOutput->setText(
        QLocale().toString(data.month_pay_first, 7));
  } else {
    ui->monthPaymentLabelOutput->setText(
        QLocale().toString(data.month_pay_first, 7) + " ... " +
        QLocale().toString(data.month_pay_last, 7));
  }
...
void CreditCalc::on_diffButton_clicked() {
  ...
  if (data.CreditDifferentiated(ui->amountCreditDoubleSpinBox->value(),
                                count_month,
                                ui->procentageCreditDoubleSpinBox->value())) {
    GetValuesFromData();
  } else {
...
void CreditCalc::on_AnnButton_clicked() {
  ...
  if (data.CreditAnnuity(ui->amountCreditDoubleSpinBox->value(), count_month,
                         ui->procentageCreditDoubleSpinBox->value())) {
    GetValuesFromData();
  } else {
    QMessageBox::warning(this, "Ошибка",
