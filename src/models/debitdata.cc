...
bool DebitData::DepositCalculation(
    std::vector<std::pair<int, double>> &array_insert,
    std::vector<std::pair<int, double>> &array_erase, double original_amount,
    double term, double interest, double tax, int accrual_period,
    bool capitalization) {
  interest_charges = 0;
  ...
  for (int i = 1, last_accrual_period = 0; i <= term; ++i) {
    ...
    original_amount -=
        tmp_amount > original_amount ? original_amount : tmp_amount;
    if (capitalization) {
      if (i % accrual_period == 0) {
        interest_charges +=
            interest * accrual_period * (original_amount + interest_charges);
        last_accrual_period = i;
      } else if (i == term) {
        int delta_accrual_period = i - last_accrual_period;
        interest_charges += interest * delta_accrual_period *
                            (original_amount + interest_charges);
      }
    } else {
  ...
  tax_amount =
      ((interest_charges / term) - (1000000 * key_rate / 12.)) * term * tax;
